...
void Audio::generateDualTone(short *buffer, short *left_buffer, short *right_buffer, double left_freq, double right_freq, int frames) {
    const double amplitude = 0.5 * 32767.0; // 避免削顶
...
void Audio::saveBufferToWav(const char *filename, short *buffer, int frames) {
    FILE *file = fopen(filename, "wb");
    if (!file) {
        fprintf(stderr, "无法打开文件用于写入: %s\n", filename);
        return;
    }
    ...
    fclose(file);
    LogInfo(AUDIO_TAG, "音频数据已保存到文件: %s (帧数: %d)", filename, frames);
}
...
// 计算信号RMS（均方根值）- 用于衡量信号强度
double Audio::calculateSignalRMS(short *signal, int samples) {
    double sum_squares = 0.0;
    int valid_samples = 0;
    
    for (int i = 0; i < samples; i++) {
        double sample = signal[i] / 32768.0; // 归一化到[-1, 1]
...
// 计算信号峰值 - 用于更精确的增益计算
double Audio::calculateSignalPeak(short *signal, int samples) {
    double max_peak = 0.0;
    
    for (int i = 0; i < samples; i++) {
        double abs_sample = fabs(signal[i] / 32768.0); // 归一化到[0, 1]
...
// 计算自动增益值 - 改进版本
double Audio::calculateAutoGain(double play_rms, double record_rms,
...
// 应用增益到音频缓冲区
void Audio::applyGainToBuffer(short *buffer, int samples, double gain) {
    for (int i = 0; i < samples; i++) {
...
void Audio::recordAndCompare(int time) {
    ...
    // 扫描播放音频参数
    printf("扫描播放音频参数...\n");
    double play_rms = calculateSignalRMS(tone_buffer, tone_frames * CHANNELS);
    double play_peak = calculateSignalPeak(tone_buffer, tone_frames * CHANNELS);
    ...
    std::thread playThread([this, tone_buffer, tone_frames, &mtx, &cv, &ready_to_start]() {
    ...
    // 创建线程但不立即获取锁
    std::thread recordThread([this, time, recorded_buffer, &mtx, &cv, &ready_to_start]() {
        // 在lambda内部获取锁并等待
        std::unique_lock<std::mutex> lock(mtx);
        cv.wait(lock, [&ready_to_start] { return ready_to_start; });
    ...
    // 扫描录制音频参数
    double record_rms = calculateSignalRMS(recorded_buffer, tone_frames * CHANNELS);
    double record_peak = calculateSignalPeak(recorded_buffer, tone_frames * CHANNELS);
    
    printf("录制音频: RMS=%.6f, 峰值=%.6f\n", record_rms, record_peak);
    
    // 自动计算增益
    double auto_gain = calculateAutoGain(play_rms, record_rms, play_peak, record_peak);
    ...
    // 应用增益到调整缓冲区
    printf("应用增益到录制音频...\n");
    applyGainToBuffer(adjusted_buffer, tone_frames * CHANNELS, auto_gain);
    
    // 计算调整后音频的RMS和峰值
    double adjusted_rms = calculateSignalRMS(adjusted_buffer, tone_frames * CHANNELS);
    double adjusted_peak = calculateSignalPeak(adjusted_buffer, tone_frames * CHANNELS);
    ...
    // 保存音频文件
    printf("\n=== 生成音频文件 ===\n");
    saveBufferToWav("recorded_audio.wav", recorded_buffer, tone_frames);
    saveBufferToWav("reference_tone.wav", tone_buffer, tone_frames);
    saveBufferToWav("adjusted_audio.wav", adjusted_buffer, tone_frames);
...
int Audio::recordAndCompareV2(int time) {
    // char * device = "plughw:rockchipes8389"; // 使用ES8388声卡设备名称
    ...
    generateDualTone(play_tone_buffer, play_left_buffer, play_right_buffer, left_freq, right_freq, tone_frames);

    LogInfo(AUDIO_TAG, "生成双声道测试音频，左声道: %.2f Hz, 右声道: %.2f Hz", left_freq, right_freq);
    saveBufferToWav("dual_tone.wav", play_tone_buffer, tone_frames);
    saveBufferToWav("play_left_channel.wav", play_left_buffer, tone_frames);
    saveBufferToWav("play_right_channel.wav", play_right_buffer, tone_frames);
    ...
    std::thread playThread([this, play_tone_buffer, tone_frames, &mtx, &cv, &ready_to_start]() {
        std::unique_lock<std::mutex> lock(mtx);
        cv.wait(lock, [&ready_to_start] { return ready_to_start; });
        lock.unlock(); // 释放锁后再执行播放
    ...
    saveBufferToWav("recorded_dual_tone.wav", recorded_buffer, tone_frames);

    double play_rms = calculateSignalRMS(play_tone_buffer, tone_frames * CHANNELS);
    double record_rms = calculateSignalRMS(recorded_buffer, tone_frames * CHANNELS);
    double play_peak = calculateSignalPeak(play_tone_buffer, tone_frames * CHANNELS);
    double record_peak = calculateSignalPeak(recorded_buffer, tone_frames * CHANNELS);
    double auto_gain = calculateAutoGain(play_rms, record_rms, play_peak, record_peak);
    applyGainToBuffer(recorded_buffer, tone_frames * CHANNELS, auto_gain);
    saveBufferToWav("adjusted_dual_tone.wav", recorded_buffer, tone_frames);
    ...
    cout << "Loaded " << samples.size() << " samples, SR=" << sr << endl;

    return detect_main_frequencies(samples, sr, topK);
}
                             ...
                             uint32_t sr, int topK)
{
    ...
    cout << "Top " << topK << " frequencies:\n";
    for (int i = 0; i < topK && i < peaks.size(); i++) {
    ...
    for (int i = 0; i < topK && i < peaks.size(); i++) {
        ...
        if (freq >= left_freq - 20 && freq <= left_freq + 20) {
            LogInfo(AUDIO_TAG, "检测到左声道频率: %.2f Hz", freq);
            left_found = 0x02;  // 0000 0010
        }
        if (freq >= right_freq - 20 && freq <= right_freq + 20) {
            LogInfo(AUDIO_TAG, "检测到右声道频率: %.2f Hz", freq);

...
bool Serial::serialCanTest(std::vector<std::string>& deviceList, std::vector<bool>& sendResult, std::vector<bool>& recvResult, int testCount) {
    ...
    for (int i = 0; i < deviceList.size(); i++) {
        ...
        ret = system(("ip link set " + deviceList[i] + " type can bitrate 500000").c_str());
        if (ret != 0) {
            sendResult[i] = false;
            recvResult[i] = false;
    ...
    for (int i = 0; i < deviceList.size(); i++) {       // 每个设备依次发送数据
        ...
        for (int j = 0; j < deviceList.size(); j++) {   // 其他设备接收数据
            ...
            std::cout << "CAN Device " << deviceList[i] << " sending frame to Device " << deviceList[j] << std::endl;
...
int Serial::open_can_device(const char* can_interface_name) {
    if (!can_interface_name) {
        log_thread_safe(LOG_LEVEL_INFO, SERIAL_TAG, "CAN interface name is null!");
        return -1;
    }
    ...
    if (system(cmd) != 0) {
        log_thread_safe(LOG_LEVEL_INFO, SERIAL_TAG, "Failed to bring down CAN interface");
        return -1;
    }

    snprintf(cmd, sizeof(cmd), "ip link set %s type can bitrate 500000", can_interface_name);
    if (system(cmd) != 0) {
        log_thread_safe(LOG_LEVEL_INFO, SERIAL_TAG, "Failed to set CAN bitrate");
        return -1;
    }

    snprintf(cmd, sizeof(cmd), "ip link set %s up", can_interface_name);
    if (system(cmd) != 0) {
        std::cerr << "Failed to bring up CAN interface" << std::endl;
        log_thread_safe(LOG_LEVEL_INFO, SERIAL_TAG, "Failed to bring up CAN interface");
        return -1;
    }

    int sock_fd = socket(PF_CAN, SOCK_RAW, CAN_RAW);
    if (sock_fd < 0) {
        log_thread_safe(LOG_LEVEL_INFO, SERIAL_TAG, "socket : %s", strerror(errno));
        return -1;
    }    

    struct ifreq ifr;
    std::memset(&ifr, 0, sizeof(ifr));
    std::strncpy(ifr.ifr_name, can_interface_name, IFNAMSIZ - 1);

    if (ioctl(sock_fd, SIOCGIFINDEX, &ifr) < 0) {
        log_thread_safe(LOG_LEVEL_INFO, SERIAL_TAG, "ioctl SIOCGIFINDEX : %s", strerror(errno));
        close(sock_fd);
        return -1;
    }
    ...
    if (bind(sock_fd, (struct sockaddr*)&addr, sizeof(addr)) < 0) {
        log_thread_safe(LOG_LEVEL_INFO, SERIAL_TAG, "bind : %s", strerror(errno));
        close(sock_fd);
        return -1;
    }
    ...
    if (flags == -1) {
        log_thread_safe(LOG_LEVEL_INFO, SERIAL_TAG, "get socket flags failed for %s", can_interface_name);
        close(sock_fd);
        return -1;
    }

    if (fcntl(sock_fd, F_SETFL, flags | O_NONBLOCK) == -1) {
        log_thread_safe(LOG_LEVEL_INFO, SERIAL_TAG, "set socket non-blocking failed for %s", can_interface_name);
        close(sock_fd);
        return -1;
    }
...
bool Serial::can_send(int fd, unsigned int can_id, unsigned char* buf) {
    if (fd < 0) {
        log_thread_safe(LOG_LEVEL_INFO, SERIAL_TAG, "Invalid CAN file descriptor");
        return false;
    }
    ...
    int nbytes = write(fd, &frame, sizeof(frame));
    if (nbytes != sizeof(frame)) {
        log_thread_safe(LOG_LEVEL_INFO, SERIAL_TAG, "CAN write failed ： %s", strerror(errno));
        return false;
    }
...
bool Serial::can_recv(int fd, unsigned char* buf) {
    if (fd < 0 || buf == nullptr) {
        log_thread_safe(LOG_LEVEL_INFO, SERIAL_TAG, "Invalid fd or buf");
        return false;
    }
    ...
    if (nbytes < 0) {
        // log_thread_safe(LOG_LEVEL_INFO, SERIAL_TAG,  "CAN read failed ： %s", strerror(errno));
        return false;
    } else if (nbytes < sizeof(struct can_frame)) { 
        log_thread_safe(LOG_LEVEL_INFO, SERIAL_TAG,  "Incomplete CAN frame received");
        return false;
    }

    std::memcpy(buf, frame.data, frame.can_dlc);
    buf[frame.can_dlc] = '\0';
    log_thread_safe(LOG_LEVEL_INFO, SERIAL_TAG, "Received CAN ID: 0x%x, recv buf: %s", frame.can_id, buf);
...
bool Serial::can_test(int fd, int test_count) {
    if (fd < 0) {
        log_thread_safe(LOG_LEVEL_INFO, SERIAL_TAG, "can test: Invalid CAN file descriptor");
        return false;
    }

    for (int i = 0; i < test_count; i++) {
        bool ret = can_send(fd, 0x001, (unsigned char *)"TESTDATA");
        if (!ret) {
            continue;
        }

...
int VendorStorage::vendor_storage_read_test(uint16_t id, uint8_t *data, uint16_t *len) {
    int ret, sys_fd;
...
int VendorStorage::vendor_storage_write_test(uint16_t id, const uint8_t *data, uint16_t len) {
    int ret, sys_fd;
    uint8_t p_buf[2048]; /* malloc req buffer or used extern buffer */
    struct rk_vendor_req *req;
...
int VendorStorage::vendor_storage_write_ln(const char *ln_string) {
    ...
    return vendor_storage_write_test(VENDOR_CUSTOM_ID_1, (const uint8_t*)ln_string, len); 
}
...
int VendorStorage::vendor_storage_write_sn(const char *sn_string) {
    ...
    return vendor_storage_write_test(VENDOR_SN_ID, (const uint8_t*)sn_string, len);
}
...
int VendorStorage::vendor_storage_write_mac(uint16_t mac_id, const uint8_t *mac_addr, uint8_t addr_len) {
    ...
    return vendor_storage_write_test(mac_id, mac_addr, addr_len);
}
#include "project/BoardFactory.h"
#include "project/CM3588S2/CM3588S2.h"
#include "project/CM3588V2_CMD3588V2/CM3588V2_CMD3588V2.h"
#include "project/CM3576/CM3576.h"
#include "common/Constants.h"
#include "Uart.h"
#include "task/TaskQueue.h"
#include "task/TaskHandler.h"
#include "util/Timer.h"
#include "util/ZenityDialog.h"
#include "util/theradpoolv1/thread_pool.h"
#include "protocol/ProtocolParser.h"
#include "hardware/RkGenericBoard.h"
#include "project/BoardFactory.h"

#include "util/Log.h"

const char *BOARD_FACTORY_TAG = "BoardFactory";
  
extern std::vector<std::shared_ptr<interrupt_flag>> interrupt_flags_vector;
extern thread_pool work_thread;

std::unique_ptr<RkGenericBoard> BoardFactory::create_board(const std::string& board_name) {
    if (board_name == "CM3588S2") {
        log_thread_safe(LOG_LEVEL_INFO, BOARD_FACTORY_TAG, "Creating CM3588S2 board instance");
        return std::make_unique<CM3588S2>();
    } else if (board_name == "CM3588V2_CMD3588V2"){
        log_thread_safe(LOG_LEVEL_INFO, BOARD_FACTORY_TAG, "Creating CM3588V2_CMD3588V2 board instance");
        return std::make_unique<CM3588V2_CMD3588V2>(
            "/sys/class/block/mmcblk0/size", "/sys/class/block/nvme0n1/size",                                       /* Storage  : emmc path, pcie path*/
            "/sys/class/block/mmcblk1/size",                                                                        /* Tf       : tf card path */
            "/sys/class/leds/led_red/brightness", LED_OFF, "/sys/class/leds/led_green/brightness", LED_OFF,         /* Led      : rled path, rled status, gled path, gled status */
            "/proc/device-tree/hw-board/hw-version",                                                                /* BaseInfo : hwid path */
            "rockchip-es8389 Headset", "plughw:rockchipes8389" ,                                                    /* audio    : audio event name */
            "/sys/class/misc/fan-ctl/pwr_en", FAN_OFF,                                                              /* Fan      : fan ctrl path, initial status */
            "wlan0", 5,                                                                                             /* Wifi     : interface, scan count */
            "/dev/rtc0"                                                                                             /* Rtc      : rtc device path */
        );
    } else if (board_name == "CM3576") {
        log_thread_safe(LOG_LEVEL_INFO, BOARD_FACTORY_TAG, "Creating CM3576 board instance");
        return std::make_unique<CM3576>(
            "/sys/class/block/mmcblk0/size", "/sys/class/block/nvme0n1/size",                                       /* Storage  : emmc path, pcie path*/
            "/sys/class/block/mmcblk1/size",                                                                        /* Tf       : tf card path */
            "/sys/class/leds/led_red/brightness", LED_OFF, "/sys/class/leds/led_green/brightness", LED_OFF,         /* Led      : rled path, rled status, gled path, gled status */
            "/proc/device-tree/hw-board/hw-version",                                                                /* BaseInfo : hwid path */
            "rockchip-es8389 Headset", "plughw:rockchipes8389" ,                                                    /* audio    : audio event name */
            "/sys/class/misc/fan-ctl/pwr_en", FAN_OFF,                                                              /* Fan      : fan ctrl path, initial status */
            "wlan0", 5,                                                                                             /* Wifi     : interface, scan count */
            "/dev/rtc0"                                                                                             /* Rtc      : rtc device path */
        );
    } else {
        std::cerr << "Unknown board type: " << board_name << std::endl;
        return nullptr;
    }
}

std::unique_ptr<RkGenericBoard> BoardFactory::create_board_v1(const std::string& board_name) {
    BOARD_NAME board_name_enum = BoardFactory::string_to_enum(board_name);
    switch(board_name_enum) {
        case BOARD_NAME::BOARD_CM3588S2: {
            log_thread_safe(LOG_LEVEL_INFO, BOARD_FACTORY_TAG, "Creating CM3588S2 board instance");
            return nullptr;
        } break;

        case BOARD_NAME::BOARD_CM3588V2_CMD3588V2: {
            log_thread_safe(LOG_LEVEL_INFO, BOARD_FACTORY_TAG, "Creating CM3588V2_CMD3588V2 board instance");
            uart = new Uart("/dev/ttyS0");
            if (!uart->open()) {
                log_thread_safe(LOG_LEVEL_ERROR, BOARD_FACTORY_TAG, "cannot open uart, exiting program");
                delete uart;
                return nullptr;
            }

            if (!uart->configure(115200, 8, 'N', 1)) {
                log_thread_safe(LOG_LEVEL_ERROR, BOARD_FACTORY_TAG, "cannot configure uart, exiting program");
                uart->close();
                delete uart;
                return nullptr;
            }
            std::unique_ptr<RkGenericBoard> board = std::make_unique<CM3588V2_CMD3588V2>(
                "/sys/class/block/mmcblk0/size", "/sys/class/block/nvme0n1/size",                                       /* Storage  : emmc path, pcie path*/
                "/sys/class/block/mmcblk1/size",                                                                        /* Tf       : tf card path */
                "/sys/class/leds/led_red/brightness", LED_OFF, "/sys/class/leds/led_green/brightness", LED_OFF,         /* Led      : rled path, rled status, gled path, gled status */
                "/proc/device-tree/hw-board/hw-version",                                                                /* BaseInfo : hwid path */
                "rockchip-es8389 Headset", "plughw:rockchipes8389" ,                                                    /* audio    : audio event name */
                "/sys/class/misc/fan-ctl/pwr_en", FAN_OFF,                                                              /* Fan      : fan ctrl path, initial status */
                "wlan0", 5,                                                                                             /* Wifi     : interface, scan count */
                "/dev/rtc0"                                                                                             /* Rtc      : rtc device path */
            );

            std::shared_ptr<interrupt_flag> led_shink_flag = 
                work_thread.submit_interruptible([board_ptr = board.get()](interrupt_flag& flag) {                                                                                                              // board_ptr 指向的是堆区对象，后面std::move(board);了。 
                                                                                                                                                                                                                // 外面函数千万不要reset这个指针，让它自然析构，设计缺陷请后面再优化   /(ㄒoㄒ)/~~
                                                                                                                                                                                   // 目前情况是大部分线程持有的board指针都是这个board_ptr指针，而线程池只有在程序退出时才析构，所以不会有野指针问题。
                    while (!flag.is_stop_requested()) {
                        board_ptr->setRledStatus(LED_ON);
                        sleep(1);
                        board_ptr->setRledStatus(LED_OFF);
                        sleep(1);
                    }
                    log_thread_safe(LOG_LEVEL_INFO, BOARD_FACTORY_TAG, "led shink task exit, thread end");
                });
            interrupt_flags_vector.push_back(led_shink_flag);

            std::shared_ptr<interrupt_flag> audio_test_flag = 
                work_thread.submit_interruptible([board_ptr = board.get()](interrupt_flag& flag) {
                    if (board_ptr->findEventDevice() == false) {
                        log_thread_safe(LOG_LEVEL_ERROR, BOARD_FACTORY_TAG, "cannot find audio input device, audio test function unavailable");
                        return;
                    }
                    while (!flag.is_stop_requested()) {
                        board_ptr->selectAudioIn();                                                                                                                                                              // 这里一定不能堵塞哟 ！！！！！！！！！ ~(≧▽≦)~
                    }
                    log_thread_safe(LOG_LEVEL_INFO, BOARD_FACTORY_TAG, "audio test task exit, thread end");
                });
            interrupt_flags_vector.push_back(audio_test_flag);

            return std::move(board);
        }

        case BOARD_NAME::BOARD_CM3576:{
            uart = new Uart("/dev/ttyUSB0");
            if (!uart->open()) {
                log_thread_safe(LOG_LEVEL_ERROR, BOARD_FACTORY_TAG, "无法打开串口，程序退出");
                delete uart;
                return nullptr;
            }

            if (!uart->configure(115200, 8, 'N', 1)) {
                log_thread_safe(LOG_LEVEL_ERROR, BOARD_FACTORY_TAG, "无法配置串口，程序退出");
                uart->close();
                delete uart;
                return nullptr;
            }

            log_thread_safe(LOG_LEVEL_INFO, BOARD_FACTORY_TAG, "Creating CM3576 board instance");
            std::unique_ptr<RkGenericBoard> board = std::make_unique<CM3576>(
                "/sys/class/block/mmcblk0/size", "/sys/class/block/nvme0n1/size",                                       /* Storage  : emmc path, pcie path*/
                "/sys/class/block/mmcblk1/size",                                                                        /* Tf       : tf card path */
                "/sys/class/leds/led_red/brightness", LED_OFF, "/sys/class/leds/led_green/brightness", LED_OFF,         /* Led      : rled path, rled status, gled path, gled status */
                "/proc/device-tree/hw-board/hw-version",                                                                /* BaseInfo : hwid path */
                "rockchip-es8389 Headset", "plughw:rockchipes8389" ,                                                    /* audio    : audio event name */
                "/sys/class/misc/fan-ctl/pwr_en", FAN_OFF,                                                              /* Fan      : fan ctrl path, initial status */
                "wlan0", 5,                                                                                             /* Wifi     : interface, scan count */
                "/dev/rtc0"                                                                                             /* Rtc      : rtc device path */
            );

            std::shared_ptr<interrupt_flag> led_shink_flag = 
                work_thread.submit_interruptible([board_ptr = board.get()](interrupt_flag& flag) {                      // board_ptr 指向的是堆区对象，后面std::move(board);了。 
                                                                                                                        // 外面函数千万不要reset这个指针，让它自然析构，设计缺陷请后面再优化   /(ㄒoㄒ)/~~
                                                                                                                        // 目前情况是大部分线程持有的board指针都是这个board_ptr指针，而线程池只有在程序退出时才析构，所以不会有野指针问题。
                    while (!flag.is_stop_requested()) {
                        board_ptr->setRledStatus(LED_ON);
                        sleep(1);
                        board_ptr->setRledStatus(LED_OFF);
                        sleep(1);
                    }
                });
            interrupt_flags_vector.push_back(led_shink_flag);

            std::shared_ptr<interrupt_flag> audio_test_flag = 
                work_thread.submit_interruptible([board_ptr = board.get()](interrupt_flag& flag) {
                    if (board_ptr->findEventDevice() == false) {
                        log_thread_safe(LOG_LEVEL_ERROR, BOARD_FACTORY_TAG, "未找到音频输入设备，音频测试功能不可用");
                        return;
                    }
                    while (!flag.is_stop_requested()) {
                        board_ptr->selectAudioIn();                  // 这里一定不能堵塞哟 ！！！！！！！！！ ~(≧▽≦)~
                    }
                });
            interrupt_flags_vector.push_back(audio_test_flag);

            return std::move(board);
        } break;

        default:
            log_thread_safe(LOG_LEVEL_ERROR, BOARD_FACTORY_TAG, "Unknown board name string: %s", board_name.c_str());
            return nullptr;
    }
}

BOARD_NAME BoardFactory::string_to_enum(const std::string& board_name_str) {
    if (board_name_str == "CM3588S2") {
        return BOARD_NAME::BOARD_CM3588S2;
    } else if (board_name_str == "CM3588V2_CMD3588V2") {
        return BOARD_NAME::BOARD_CM3588V2_CMD3588V2;
    } else if (board_name_str == "CM3576") {
        return BOARD_NAME::BOARD_CM3576;
    } else {
        log_thread_safe(LOG_LEVEL_ERROR, BOARD_FACTORY_TAG, "Unknown board name string: %s", board_name_str.c_str());
        return BOARD_NAME::BOARD_UNKNOWN;
    }
}

BoardFactory::~BoardFactory() {
    if (uart) {
        uart->close();
        delete uart;
        uart = nullptr;
    }
}
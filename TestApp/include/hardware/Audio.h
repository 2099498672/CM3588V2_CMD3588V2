...
public:
    ...
    int recordAndCompareV2(int time);
    
    // 工具函数
    void generate_tone(short *buffer, int frames);
    void saveBufferToWav(const char *filename, short *buffer, int frames);
    // 生成指定频率的双声道音频（左右声道不同频率）
    void generateDualTone(short *buffer, short *left_buffer, short *right_buffer, double left_freq, double right_freq, int frames);
...
private:
    ...
    // 自动增益控制函数
    double calculateSignalRMS(short *signal, int samples);
    double calculateSignalPeak(short *signal, int samples);
    double calculateAutoGain(double play_rms, double record_rms, double play_peak, double record_peak);
    void applyGainToBuffer(short *buffer, int samples, double gain);
